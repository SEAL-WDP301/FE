...
import { JudgeCommentsCard } from "./components/judge-comments-card";
...
export default function EvaluationPage() {
  ...
  useEffect(() => {
    ...
    params.set("roundId", String(selectedRoundId));
    router.replace(`/judge/evalution?${params.toString()}`, { scroll: false });
    ...
    mutationFn: async () => {
      ...
      const payload = {
        scores: submissionDetail.rubrics.map((rubric) => {
          const criterionComment = comments[rubric.id]?.trim() ?? "";
          const mergedComment = overallComment.trim()
            ? `[General] ${overallComment.trim()}${criterionComment ? `\n${criterionComment}` : ""}`
            : criterionComment;
  ...
  const handleSelectRound = useCallback(
    (roundId: number) => {
      ...
      router.replace(`/judge/evalution?${params.toString()}`);
      setSelectedSubmissionId(null);
    },
